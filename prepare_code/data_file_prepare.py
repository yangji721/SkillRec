...
def create_top_k_frequent_skill_set():
    return heapq.nlargest(30, range(len(skill_cnt)), skill_cnt.__getitem__)
...
def create_data_with_neg(sequence_list, direct_name):
    ...
    for lst_id, skill_lst in enumerate(sequence_list[297:]):
        ...
        preference = random.sample(top_k_set, 8)
        ...
        for skill in skill_lst[1:]:
            ...
            easy_pos, salary_pos, _, _, _= environment.add_skill(skill, evaluate=True)

            vis[skill] = 1
            neg_skills.append(s)
            negr_lst.append((easy_neg, salary_neg))
            r_lst.append((easy_pos, salary_pos))
        data.append(((skill_lst, preference), r_lst, neg_skills, negr_lst))
...
if __name__ == "__main__":
    ...
    create_data_with_neg(resume_list, "preference")
    validation_split("preference")
