...
class OnPolicyTrainer(object):
    ...
    def train(self, n_batch, batch_size, data_train, data_valid, save_path, verbose_batch=500, T=26):
        ...
        for it in tqdm(range(n_batch)):
            ...
            for t in range(T):
                ...
                self.memory.store((state_pre, s, (easy, salary - salary_pre), preference))  # 要有遗忘才行
                salary_pre = salary
    ...
    def transform_train_batch(self, data_batch):
        ...
        if self.md == 'salary':
            return data_state, data_skill, data_salary, data_preference
        elif self.md == 'easy':
            return data_state, data_skill, data_easy, data_preference
        else:
            return data_state, data_skill, data_r, data_preference
...
#    env_params = {"lambda_d": 0.1, "beta": 0.2, 'pool_size': 50}
    ...
    on_trainer.train(n_batch=480000, batch_size=128, data_train=data_train, data_valid=data_valid, verbose_batch=10240,
                     save_path=HOME_PATH + "data/prefermodel/%s_DNN_%s_exp" % (direct_name, md), T=20)
    ...
    # ----------------- 模型保存 -----------------------
    Qa.save(HOME_PATH + "data/prefermodel/%s_DNN_%s_exp" % (direct_name, md))