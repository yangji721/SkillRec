...
#    env_params = {"lambda_d": 0.1, "beta": 0.2, 'pool_size': 50}
    ...
    on_trainer.train(n_batch=480000, batch_size=4096, data_train=data_train, data_valid=data_valid, verbose_batch=10240,
                     save_path=HOME_PATH + "data/model1/%s_DNN_%s_exp1" % (direct_name, md), T=20)
    ...
    # ----------------- 模型保存 -----------------------
    Qa.save(HOME_PATH + "data/model1/%s_DNN_%s_exp1" % (direct_name, md))
