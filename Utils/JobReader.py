...
def _get_similarity(job1, job2, a, b):
    set1, set2 = set(job1), set(job2)
    intersection_set = [value for value in set1 if value in set2]
    set1_diff = set1.difference(set2)
    set2_diff = set2.difference(set1)
    similarity = len(intersection_set)/(len(intersection_set) + a*len(set1_diff) + b*len(set2_diff))
    return similarity

def get_similarity_score(sample_lst, target, a, b):
    result = []
    for sample in sample_lst:
        result.append(_get_similarity(target, sample[0], a, b))
    return result

def get_set_distribution_0_1(data):
    distribution = [0] * 10
    for item in data:
        distribution[int(item//0.1)] += 1
    return distribution
