...
def evaluate(sampler, environment, data_test, train_samples, epoch, T=16, verbose=False):
    ...
    for x, y in data_test:
        ...
        for j, k in enumerate(range(1, T + 1)):
            ...
            step_salary_base[j] += step_metrics[k - 1][1] / N_test
            step_similarity_base[j] += step_metrics[k - 1][2] / N_test

    time_end = time.time()
    sec = time_end - time_start
    # 输出
    print("[time:%d, epoch %d] avg_easy: %.4f, avg_salary: %.4f, salary_start: %.4f\n , avg_salary_base: %.4f\n, avg_similarity_base: %.4f\n , step_salary: " % (sec, epoch, avg_easy, avg_salary, avg_start, avg_salary_base, avg_similarity_base), end="")
